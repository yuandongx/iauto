...
import { Modal, Input, Card, Button, Radio, Form } from 'antd';
import store from './store';

export const Address = ({form}) => {
    const [radioValue, setRadioValue] = React.useState(1);
    return (
    <Card actions={[<PlusOutlined key="add" />]}>
      <Form labelCol={{ span: 5 }}
            ...
            <Form.Item label="类型" name="kind" rules={[{ required: true, message: '请选择要配置的对象类型!' }]}>
              <Radio.Group onChange={(v)=> setRadioValue(v)}>
                    <Radio value={1}>主机地址</Radio>
                    <Radio value={2}>地址范围</Radio>
                    <Radio value={3}>地址子网</Radio>
                  </Radio.Group>
            </Form.Item>
            {radioValue === 1 && <Form.Item required label="IP地址" rules={[{ required: true, message: '请输入主机地址!' }]}>
